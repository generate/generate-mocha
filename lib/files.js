...
function install(app, cb) {
  return function() {
    var devDeps = Object.keys(app.pkg.get('devDependencies') || {});
    var queue = (app.get('cache.install') || []).filter(function(name) {
      return devDeps.indexOf(name) === -1;
    });

    if (queue.length) {
      app.npm.askInstall(queue, cb);
    } else {
      cb();
    }
  }
